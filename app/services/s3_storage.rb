...
class S3Storage
  ...
  def self.store(key, data)
    ...
    date_key = OpenSSL::HMAC.digest("SHA256", "AWS4#{SECRET_KEY}", date)
    ...
    # Check if the HTTP status code is exactly "200" (Success)
    if response.code == "200"
      true
    else
      # If S3 rejects it, log S3's error message and return false
      Rails.logger.error("S3 Upload Failed: [HTTP #{response.code}] #{response.body}")
      false
    end
  rescue StandardError => e
    # Catch pure network failures (like if the computer loses internet)
