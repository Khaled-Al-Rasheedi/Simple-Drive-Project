...
#Step 5: If the blob was saved to one of the backends destroy the blob    
   ...
   blob = Blob.create(
    ...
    if blob.persisted? #Check if creation of blob was successful
        ...
        storage_success=StorageRouter.storage(req_id, req_data)#To store the the blob
    if storage_success
      ...
      end #End of check if storage was successful

    else
      #This is for checking if blob was savedd to the main database.
...
#Above we get the blob id then check if its exists, if not raise an error
    if blob
